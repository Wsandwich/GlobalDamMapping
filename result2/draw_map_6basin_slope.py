...
def plot_map(data, column, ax, title, output_path, gdf_policies_grouped=None, vmin=None, vmax=None, custom_bins=None):
    ...
    cbar.set_label(f'Dam Trend Slope ({column})', fontsize=10)
    
    # 添加无变化和政策类型图例
    legend_elements = [mpatches.Patch(facecolor='whitesmoke', edgecolor='none', label='No Trend (slope = 0)')]
...
# **计算线性回归斜率的函数**
def calculate_slope(values_2010, values_2015, values_2020):
    """
    计算线性回归斜率
    参数:
        values_2010, values_2015, values_2020: 三年的水坝数量数据
    返回:
        斜率值
    """
    # 时间点（以年为单位）
    ...
    slopes = []
    for i in range(len(values_2010)):
        # 当前流域三年的数据
        dam_counts = np.array([values_2010.iloc[i], values_2015.iloc[i], values_2020.iloc[i]])
        
        # 如果所有年份数据都相同，斜率为0
        if np.all(dam_counts == dam_counts[0]):
            slopes.append(0.0)
        else:
            # 使用scipy.stats.linregress计算斜率
            slope, intercept, r_value, p_value, std_err = stats.linregress(years, dam_counts)
            slopes.append(slope)
    
    return np.array(slopes)

# **自动生成颜色区间的函数**
def generate_auto_bins(data, n_bins=10):
    """
    根据数据分布自动生成颜色区间
    参数:
        data: 斜率数据
    ...
    negative_data = non_zero_data[non_zero_data < 0]
    
    bins = []
    
    # 负值区间
    if len(negative_data) > 0:
        neg_percentiles = np.linspace(0, 100, n_bins//2 + 1)
        neg_bins = np.percentile(negative_data, neg_percentiles)
        # 确保边界是唯一的并且单调递增
        neg_bins = np.unique(neg_bins)
        # 按升序排列负值（从最小到最大）
        neg_bins = np.sort(neg_bins)
        bins.extend(neg_bins)
    else:
        # 如果没有负值，添加一个小的负值边界
        bins.append(min(data.min(), -0.001))
    ...
    if len(positive_data) > 0:
        pos_percentiles = np.linspace(0, 100, n_bins//2 + 1)
        pos_bins = np.percentile(positive_data, pos_percentiles)
        # 确保边界是唯一的并且单调递增
        pos_bins = np.unique(pos_bins)
        # 按升序排列正值
        pos_bins = np.sort(pos_bins)
        # 排除0值（因为已经添加了）
        pos_bins = pos_bins[pos_bins > 0]
        bins.extend(pos_bins)
    else:
        # 如果没有正值，添加一个小的正值边界
        bins.append(max(data.max(), 0.001))
    
    # 确保最终的bins是单调递增的
    bins = np.array(bins)
    bins = np.unique(bins)  # 移除重复值
    bins = np.sort(bins)    # 确保单调递增
    
    # 验证bins的有效性
    if len(bins) < 2:
        print("警告：生成的区间数量不足，使用默认区间")
        return np.array([data.min(), 0, 0, data.max()])
    
    # 检查是否有连续的0
    zero_indices = np.where(bins == 0)[0]
    if len(zero_indices) >= 2:
        # 保持第一个0，将后续的0稍微调整
        for i in range(1, len(zero_indices)):
            bins[zero_indices[i]] = 1e-10
...
output_base_path = r'E:\wyj\dam\v4\image\result2\images_trend_analysis_06'
csv_output_dir = os.path.join(output_base_path, 'trend_statistics')
...
# **计算四种水坝类型的趋势斜率（基于2010、2015、2020三年数据）**
print("计算各类型水坝的趋势斜率...")
for code, full_name in dam_type_mapping.items():
    print(f"计算 {full_name} 水坝趋势斜率...")
    
    # 获取三年的数据
    values_2010 = basins[f'2010_{code}']
    values_2015 = basins[f'2015_{code}']
    values_2020 = basins[f'2020_{code}']
    
    # 计算斜率
    slope_col = f'slope_{code}'
    basins[slope_col] = calculate_slope(values_2010, values_2015, values_2020)
    
    print(f"{full_name} 水坝趋势斜率范围: [{basins[slope_col].min():.4f}, {basins[slope_col].max():.4f}]")
    print(f"{full_name} 水坝趋势斜率非零值数量: {len(basins[basins[slope_col] != 0])}")

# **计算总体趋势斜率**
print("计算总体趋势斜率...")
slope_columns = [f'slope_{code}' for code in dam_type_mapping.keys()]

# 计算总体斜率（通过计算总数的斜率，而不是各个斜率的和）
total_2010 = basins[[f'2010_{code}' for code in dam_type_mapping.keys()]].sum(axis=1)
total_2015 = basins[[f'2015_{code}' for code in dam_type_mapping.keys()]].sum(axis=1)
total_2020 = basins[[f'2020_{code}' for code in dam_type_mapping.keys()]].sum(axis=1)

basins['slope_total'] = calculate_slope(total_2010, total_2015, total_2020)

print(f"总体趋势斜率范围: [{basins['slope_total'].min():.4f}, {basins['slope_total'].max():.4f}]")
print(f"总体趋势斜率非零值数量: {len(basins[basins['slope_total'] != 0])}")
...
def generate_bin_statistics(data, column, bin_edges, output_csv_path):
    ...
    if count_below > 0:
        ...
        bin_labels.append(f"< {min_edge:.4f}")
    ...
    for i in range(len(bin_edges) - 1):
        ...
        if lower == 0 and upper == 0:
            ...
            if count_neg > 0:
                ...
                bin_labels.append(f"[{lower:.4f}, 0)")
            ...
            if count_pos > 0:
                ...
                bin_labels.append(f"(0, {upper:.4f}]")
        else:
            # 常规区间 [lower, upper]
            count = len(data[(data[column] >= lower) & (data[column] <= upper)])
            if count > 0:
                bin_counts.append(count)
                bin_labels.append(f"[{lower:.4f}, {upper:.4f}]")
    ...
    if count_above > 0:
        ...
        bin_labels.append(f"> {max_edge:.4f}")
    ...
    print(f"趋势统计数据已保存至: {output_csv_path}")
...
# **绘制每种水坝类型的趋势图**
for code, full_name in dam_type_mapping.items():
    print(f"绘制 {full_name} 水坝趋势图...")
    slope_col = f'slope_{code}'

    # 创建绘图
    fig, ax = plt.subplots(figsize=(15, 10), subplot_kw={'projection': ccrs.Robinson()})
    output_path = os.path.join(output_base_path, f'dam_trend_{full_name}_2010-2020.png')
    
    # 自动生成颜色区间
    custom_bins = generate_auto_bins(basins[slope_col], n_bins=12)
    print(f"{full_name} 自动生成的区间边界: {custom_bins}")
    
    # 生成并保存区间统计
    csv_output_path = os.path.join(csv_output_dir, f'trend_statistics_{full_name}_2010-2020.csv')
    generate_bin_statistics(basins, slope_col, custom_bins, csv_output_path)
    ...
    plot_map(
        ...
        column=slope_col,
        ax=ax,
        title=f'{full_name.title()} Dam Trend (2010-2020) - Linear Regression Slope',
...
print("绘制总体趋势图...")
fig, ax = plt.subplots(figsize=(15, 10), subplot_kw={'projection': ccrs.Robinson()})
output_path_total = os.path.join(output_base_path, 'dam_trend_total_2010-2020.png')

# 自动生成总体趋势的颜色区间
custom_bins_total = generate_auto_bins(basins['slope_total'], n_bins=12)
print(f"总体趋势自动生成的区间边界: {custom_bins_total}")

# 生成并保存总体趋势的区间统计
csv_output_path_total = os.path.join(csv_output_dir, 'trend_statistics_total_2010-2020.csv')
generate_bin_statistics(basins, 'slope_total', custom_bins_total, csv_output_path_total)
...
plot_map(
    ...
    column='slope_total',
    ax=ax,
    title='Total Dam Trend (2010-2020) - Linear Regression Slope',
...
for code, full_name in dam_type_mapping.items():
    slope_col = f'slope_{code}'
    increasing = len(basins[basins[slope_col] > 0])
    decreasing = len(basins[basins[slope_col] < 0])
    no_change = len(basins[basins[slope_col] == 0])
    
    print(f"{full_name}:")
    print(f"  增长趋势流域: {increasing} ({increasing/len(basins)*100:.1f}%)")
    print(f"  下降趋势流域: {decreasing} ({decreasing/len(basins)*100:.1f}%)")
    print(f"  无变化流域: {no_change} ({no_change/len(basins)*100:.1f}%)")
    print(f"  平均斜率: {basins[slope_col].mean():.6f}")
    print()

# 总体统计
increasing_total = len(basins[basins['slope_total'] > 0])
decreasing_total = len(basins[basins['slope_total'] < 0])
no_change_total = len(basins[basins['slope_total'] == 0])

print("总体趋势:")
print(f"  增长趋势流域: {increasing_total} ({increasing_total/len(basins)*100:.1f}%)")
print(f"  下降趋势流域: {decreasing_total} ({decreasing_total/len(basins)*100:.1f}%)")
print(f"  无变化流域: {no_change_total} ({no_change_total/len(basins)*100:.1f}%)")
print(f"  平均斜率: {basins['slope_total'].mean():.6f}")