...
class Canvas(QWidget):
    ...
    scrollRequest_1 = pyqtSignal(int, int)
    scrollRequest_2 = pyqtSignal(int, int)
    newShape = pyqtSignal()
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.image = QImage()  # 您的图像


    def setZoom(self, zoom_level):
        """
        设置缩放级别并调整画布大小。
        :param zoom_level: 缩放百分比（如200表示200%）
        """
        self.scale = zoom_level / 100.0
    ...
    def wheelEvent(self, ev):
        ...
        if Qt.ControlModifier == int(mods) and v_delta:
            self.zoomRequest.emit(v_delta)  # 向 zoomRequest 信号发送 delta 和位置
        else:
            v_delta and self.scrollRequest.emit(v_delta, Qt.Vertical)
            h_delta and self.scrollRequest.emit(h_delta, Qt.Horizontal)
            # v_delta and self.scrollRequest_1.emit(v_delta, Qt.Vertical)
            # h_delta and self.scrollRequest_1.emit(h_delta, Qt.Horizontal)
            # v_delta and self.scrollRequest_2.emit(v_delta, Qt.Vertical)
            # h_delta and self.scrollRequest_2.emit(h_delta, Qt.Horizontal)
        ev.accept()
    ...
    def addShape(self, shape):
        """将新的 Shape 对象添加到 Canvas 中，并更新显示。"""
        self.shapes.append(shape)
        self.update()  # 刷新画布以显示新的标注
