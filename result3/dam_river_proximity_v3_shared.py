...
def create_index_metadata(rivers):
    metadata = [(pos, river.geometry.bounds) for pos, river in enumerate(rivers.itertuples())]
    return metadata

def rebuild_index_from_metadata(metadata):
    idx = index.Index()
    for pos, bounds in metadata:
        idx.insert(pos, bounds)
    return idx

def process_dam_batch(rivers, shm_name, batch_data, max_search_distance=None):
    """
    ...
    shm_name : str
        Name of the shared memory block containing the serialized spatial index
    batch_data : tuple
    ...
    # Access the shared memory and deserialize the spatial index
    start_time = time.time()
    try:
        existing_shm = shared_memory.SharedMemory(name=shm_name)
        # Get the size from the first 8 bytes (using int.from_bytes)
        index_size = int.from_bytes(existing_shm.buf[:8], byteorder='little')
        # Deserialize the index from the remaining bytes
        # local_idx = pickle.loads(bytes(existing_shm.buf[8:8+index_size]))
        metadata = pickle.loads(bytes(existing_shm.buf[8:8+index_size]))
        local_idx = rebuild_index_from_metadata(metadata)
        end_time = time.time()
        print(f"Accessed shared spatial index in {end_time - start_time:.2f} seconds")
    except Exception as e:
        # Fallback to creating a local index if shared memory access fails
        print(f"Error accessing shared index: {str(e)}. Creating local index instead.")
        start_time = time.time()
        local_idx = index.Index()
        for pos, river in enumerate(rivers.itertuples()):
            local_idx.insert(pos, river.geometry.bounds)
        end_time = time.time()
        print(f"Built local spatial index for {len(rivers)} rivers in {end_time - start_time:.2f} seconds")
    ...
    for i, dam in enumerate(dam_batch.itertuples()):
        ...
        if max_search_distance:
            ...
            potential_matches_idx = list(local_idx.intersection((minx, miny, maxx, maxy)))
        else:
            ...
            potential_matches_idx = list(local_idx.nearest(dam_bounds, 50))
    ...
    # Clean up the shared memory reference
    try:
        existing_shm.close()
    except:
        pass
...
def find_nearest_rivers_to_dams(dam_shp_path, river_gpkg_path, output_dam_path=None, batch_size=100, max_search_distance=None, max_workers=None):
    """
    Find the nearest river segment for each dam and calculate the distance using spatial indexing and parallel processing for efficiency.
    ...
    '''
    # Create spatial index in the main process
    print("Building spatial index for rivers...")
    start_index_time = time.time()
    rivers_idx = create_spatial_index(rivers)
    end_index_time = time.time()
    print(f"Built spatial index in {end_index_time - start_index_time:.2f} seconds")
    
    # Serialize the spatial index for shared memory
    print("Preparing shared spatial index...")
    serialized_index = pickle.dumps(rivers_idx)
    index_size = len(serialized_index)
    
    # Create shared memory block - need to include 8 bytes for size information
    shm = shared_memory.SharedMemory(create=True, size=8+index_size)
    
    # Store the size of the index at the beginning of the shared memory block
    shm.buf[:8] = index_size.to_bytes(8, byteorder='little')
    
    # Store the serialized index in the shared memory
    shm.buf[8:8+index_size] = serialized_index
    shm_name = shm.name
    print(f"Created shared memory block for spatial index: {shm_name}")
    '''
    # 在主进程中
    print("Creating index metadata...")
    index_metadata = create_index_metadata(rivers)
    serialized_metadata = pickle.dumps(index_metadata)
    index_size = len(serialized_metadata)
    shm = shared_memory.SharedMemory(create=True, size=8+index_size)
    shm.buf[:8] = index_size.to_bytes(8, byteorder='little')
    shm.buf[8:8+index_size] = serialized_metadata
    shm_name = shm.name
    ...
    try:
        with concurrent.futures.ProcessPoolExecutor(max_workers=max_workers) as executor:
            ...
            process_batch_func = partial(
                process_dam_batch, 
                rivers,  # Pass the pre-loaded rivers data to each worker
                shm_name,  # Pass the shared memory name instead of the index
                max_search_distance=max_search_distance
            )
            
            # Submit all batches and process results as they complete
            futures = {executor.submit(process_batch_func, batch): batch[0] for batch in batch_data}
            ...
            for future in tqdm(concurrent.futures.as_completed(futures), total=len(futures)):
                try:
                    ...
                    # Update dams DataFrame with results
                    ...
                    print(f"Error processing batch starting at index {futures[future]}: {str(e)}")
    finally:
...
if __name__ == "__main__":
    ...
    # Process dams in parallel with smaller batch size for better parallelization
    updated_dams = find_nearest_rivers_to_dams(
        ...
        batch_size=100,  # Smaller batch size for better parallelization
        max_search_distance=max_search_distance,
        max_workers=None  # Use all available CPU cores
    )
    
    # Create a mapping table (optional) - now using the already loaded rivers
    # This avoids another read of the rivers file
    ...
    mapping_table = create_dam_river_mapping_table(
        ...
        rivers=rivers,  # Pass the already loaded rivers
