...
# gpkg_files = gpkg_files[:2]  # Limiting to first 20 files as in original codeS
...
def process_gpkg_file(gpkg_file):
    ...
    try:
        ...
        try:
            ...
            print(f"  - {file_name}: DOF layer records: {len(dof_gdf)} (took {elapsed_time:.2f} seconds)")
            ...
            if 'DOF' in dof_gdf.columns:
                ...
                elapsed_time = time.time() - start_time
                print(f"  - {file_name}: Converted DOF column (took {elapsed_time:.2f} seconds)")
                ...
                if 'RIV_ORD' in dof_gdf.columns:
                    ...
                    elapsed_time = time.time() - start_time
                    print(f"  - {file_name}: Applied width_logic to RIV_ORD (took {elapsed_time:.2f} seconds)")
                else:
                    dof_gdf['width'] = 0.5  # Default width
                    print(f"  - {file_name}: Set default width 0.5 (no RIV_ORD column)")
                
                start_time = time.time()
                valid_dof_gdf = dof_gdf[dof_gdf.geometry.apply(lambda x: isinstance(x, MultiLineString))]
                elapsed_time = time.time() - start_time
                if len(valid_dof_gdf) < len(dof_gdf):
                    print(f"  - {file_name}: Warning: Removed {len(dof_gdf) - len(valid_dof_gdf)} non-MultiLineString geometries (took {elapsed_time:.2f} seconds)")
                else:
                    print(f"  - {file_name}: No non-MultiLineString geometries removed (took {elapsed_time:.2f} seconds)")
                
                # Ensure the GeoDataFrame has a CRS
                if valid_dof_gdf.crs is None:
                    valid_dof_gdf.crs = "EPSG:4326"  # Set to WGS 84 if missing
                    print(f"  - {file_name}: Set CRS to EPSG:4326")
                
                dof_gdf_result = valid_dof_gdf
                print(f"  - {file_name}: Processed DOF data")
        except Exception as e:
            print(f"Error processing DOF layer in file {file_name}: {str(e)}")
        
        # Process dof_all field - check if in DOF layer
        if dof_gdf is not None and 'dof_all' in dof_gdf.columns:
            ...
            dof_all_gdf_result = valid_dof_all_gdf
            print(f"  - {file_name}: Processed dof_all data from DOF layer")
        else:
                ...
                for layer in layers:
                    ...
                    try:
                        ...
                        if 'dof_all' in layer_gdf.columns:
                            print(f"  - {file_name}: Found dof_all field in layer '{layer}'")
                            ...
                            dof_all_gdf_result = valid_layer_gdf
                            print(f"  - {file_name}: Processed dof_all data from layer '{layer}'")
                            break
                    except Exception as e:
                        print(f"  - {file_name}: Error reading layer '{layer}': {str(e)}")
            except Exception as e:
                print(f"  - {file_name}: Error listing layers: {str(e)}")
    except Exception as e:
        print(f"Error processing file {file_name}: {str(e)}")
    
    return file_name, dof_gdf_result, dof_all_gdf_result
...
# Define plot function (unchanged from your original code)
...
def main():
    ...
    # Set max_workers based on CPU count - usually set to number of cores or slightly fewer
    max_workers = os.cpu_count() - 1 if os.cpu_count() > 1 else 1
    print(f"Using {max_workers} parallel processes")
    
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        # Submit all files for processing
        future_to_file = {executor.submit(process_gpkg_file, gpkg_file): gpkg_file for gpkg_file in gpkg_files}
        
        # Process results as they complete
        for future in as_completed(future_to_file):
            file_name, dof_gdf, dof_all_gdf = future.result()
            
            if dof_gdf is not None:
                all_dof_gdf_list.append(dof_gdf)
                print(f"Added DOF data from {file_name} to combined list")
                
            if dof_all_gdf is not None:
                all_dof_all_gdf_list.append(dof_all_gdf)
                print(f"Added dof_all data from {file_name} to combined list")
    
    total_elapsed = time.time() - start_time
    print(f"All files processed in {total_elapsed:.2f} seconds")
