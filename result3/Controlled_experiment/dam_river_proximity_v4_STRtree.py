...
def find_nearest_rivers_to_dams(dam_shp_path, river_gpkg_path, output_dam_path=None, batch_size=100, max_search_distance=None, max_workers=None):
    ...
    if dams.crs is None:
        ...
        rivers = gpd.read_file(river_gpkg_path, columns=['geometry'])
...
# 假设 find_nearest_rivers_to_dams 和 create_dam_river_mapping_table 函数已定义

if __name__ == "__main__":
    # 输入目录
    rivers_dir = "/mnt/bf9340de-26bc-4032-9f11-494ba8ad1b3a/wyj/data/rivers_by_basin_03"
    dams_dir = "/mnt/bf9340de-26bc-4032-9f11-494ba8ad1b3a/wyj/data/Controlled_experiment/dam/GDW"
    
    output_dams_dir = "/mnt/bf9340de-26bc-4032-9f11-494ba8ad1b3a/wyj/data/Controlled_experiment/dam_with_rivers/GDW_with_rivers_03"
    output_mapping_dir = "/mnt/bf9340de-26bc-4032-9f11-494ba8ad1b3a/wyj/data/Controlled_experiment/dam_with_rivers/GDW_river_mappings_03"

    os.makedirs(output_dams_dir, exist_ok=True)
    os.makedirs(output_mapping_dir, exist_ok=True)
    
    # 可选：最大搜索距离（单位取决于 CRS，例如米）
    max_search_distance = 5000  # 根据数据调整
    
    # 获取所有流域 ID（基于河流或水坝文件的文件名）
    river_files = [f for f in os.listdir(rivers_dir) if f.endswith('.gpkg')]
    basin_ids = [Path(f).stem for f in river_files]  # 提取 HYBAS_ID（文件名去掉扩展名）
    print(f"发现 {len(basin_ids)} 个流域需要处理。")
    
    # 初始化统计信息
    total_dams_processed = 0
    total_valid_matches = 0
    all_distances = []
    
    # 循环处理每个流域
    for basin_id in tqdm(basin_ids, desc="处理流域"):
        # 构造输入文件路径
        ...
        if not os.path.exists(river_gpkg_path):
            print(f"流域 {basin_id} 的河流文件不存在，跳过。")
            continue
        if not os.path.exists(dam_shp_path):
            print(f"流域 {basin_id} 的水坝文件不存在，跳过。")
            continue
        ...
        if os.path.exists(output_dam_path):
            print(f"流域 {basin_id} 的水坝文件已存在，跳过。")
            continue

        try:
            # 调用 find_nearest_rivers_to_dams 函数
            ...
            # 统计信息
            total_dams_processed += len(updated_dams)
            valid_distances = updated_dams['DISTANCE_TO_RIVER'][updated_dams['DISTANCE_TO_RIVER'] >= 0]
            total_valid_matches += len(valid_distances)
            all_distances.extend(valid_distances.tolist())
            
            print(f"流域 {basin_id} 处理完成：")
            print(f"  处理的水坝数量: {len(updated_dams)}")
            print(f"  成功匹配的河流数量: {len(valid_distances)}")
    ...
    # 打印总体统计信息
    print("\n总体统计信息：")
    print(f"总共处理的水坝数量: {total_dams_processed}")
    print(f"成功匹配河流的水坝数量: {total_valid_matches}")
    if all_distances:
        print(f"平均到最近河流的距离: {sum(all_distances) / len(all_distances):.2f} 地图单位")
        print(f"最大到最近河流的距离: {max(all_distances):.2f} 地图单位")
        print(f"最小到最近河流的距离: {min(all_distances):.2f} 地图单位")
    else:
        print("没有找到任何有效距离数据。")