...
def init_worker(rivers_data):
    ...
    """
    global worker_rivers, worker_idx
    
    # Store rivers data in worker's global namespace
    worker_rivers = rivers_data
    
    # Create spatial index for rivers in this worker process - once per worker
    start_time = time.time()
    worker_idx = index.Index()
    for pos, river in enumerate(worker_rivers.itertuples()):
        worker_idx.insert(pos, river.geometry.bounds)
    end_time = time.time()
    
    print(f"Worker process {os.getpid()} initialized with spatial index for {len(worker_rivers)} rivers in {end_time - start_time:.2f} seconds")

def process_dam_batch(batch_data, max_search_distance=None):
    """
    Process a batch of dams to find their nearest rivers.
    Uses the worker-level global variables worker_rivers and worker_idx.
    ...
    for i, dam in enumerate(dam_batch.itertuples()):
        ...
        # Get potential nearest rivers using worker-level spatial index
        if max_search_distance:
            ...
            potential_matches_idx = list(worker_idx.intersection((minx, miny, maxx, maxy)))
        else:
            ...
            potential_matches_idx = list(worker_idx.nearest(dam_bounds, 50))
