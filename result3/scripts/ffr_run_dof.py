...
def run_dof(stamp, para, paths):
    ...
    dam_fc:gpd.GeoDataFrame = para["dams_fc"] # DAMS
    streams_fc:gpd.GeoDataFrame = para["streams_fc"] # RIVERS

    update_stream_mode:str = para["update_mode"]

    drf_upstream = para["drf_upstream"]  # 5
    drf_downstream = para["drf_downstream"] # 5

    barrier_inc_field = para["barrier_inc_field"] 
    # 大坝数据库中的字段，用于决定哪些大坝被纳入评估。字段值为0或1，分别表示不使用或使用该障碍物进行评估
    dof_field = para["dof_field"] # 用于存储DOF值的字段名称
    dof_mode = para["dof_mode"] # 1

    use_dam_level_df = para["use_dam_level_df"] # No
    ...
    in_basins = list(get_unique(dam_fc, barrier_inc_field)) # 得到需要用于计算的流域
